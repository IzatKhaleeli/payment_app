 ...
 class AppDelegate: FlutterAppDelegate, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    var isBluetoothInitialized = false  // Flag to track Bluetooth initialization status
     ...
     override func application(
         ...
         GeneratedPluginRegistrant.register(with: self)  // Ensure plugin registration
          // Initialize Bluetooth
        if !isBluetoothInitialized {
            initializeBluetooth()        
            isBluetoothInitialized = true  // Mark Bluetooth as initialized
        }   
         ...
         channel.setMethodCallHandler { (call, result) in
            ...
            case "isConnected":
                self.isConnection(result: result)
            case "isBluetoothPoweredOn":
                self.isBluetoothPoweredOn(result: result)  // Handle the new method
             default:
    ...
    // Create the Bluetooth central manager
    ...
    // MARK: - Check if Bluetooth is powered on
    private func isBluetoothPoweredOn(result: @escaping FlutterResult) {
    ...
    if centralManager.state == .poweredOn {
        result(true)  // Bluetooth is powered on
    } else {
        result(false) // Bluetooth is not powered on
    }
}
    // MARK: - Start Scanning
    private func startScanning() {
        ...
        // Add a short delay to ensure Bluetooth stack is fully initialized
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ...
            self.scanningTimer = Timer.scheduledTimer(timeInterval: 2.5, target: self, selector: #selector(self.stopScanningTimer), userInfo: nil, repeats: true)
        }
    }
 ...
 // MARK: - Stop Scanning Timer
     ...
     // MARK: - Check Connection State
    private func isConnection(result: @escaping FlutterResult) {
        // Check if a device is connected
        if let _ = connectedPeripheral {
            result(true)  // Device is connected
        } else {
            result(false)  // Device is not connected
        }
    }

     // MARK: - Connect to Device
     private func connectToDevice(address: String, result: @escaping FlutterResult) {
        print("private func connectToDevice ${address} ");
                print(address);
